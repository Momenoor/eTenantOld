            ...
            <form wire:submit.prevent="formSubmit" onsubmit="createPageModalLoading()">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 3rem; grid-template-rows: repeat(2, 1fr);">
                        <div class="p-0 py-2">
                            ...
                            <div class="input-group">
                                ...
                                <input autofocus name="name" required wire:model="name" class="form-control @if(isset($this->errors['view'])) is-invalid @endif" placeholder="page_name" pattern=".+\w$" title="Please provide a valid path/name." />
                                <div class="input-group-append">
                        ...
                        <div class="p-0 py-2">
                            ...
                            <div class="input-group">
                                ...
                                <input name="route" wire:model="route" class="form-control @if(isset($this->errors['route'])) is-invalid @endif" />
                                <div class="invalid-feedback">
                                    {{ $this->errors['route'] ?? '' }}
                                </div>
                            </div>
                        </div>
                        <div class="text-{{ $this->routeEdited ? 'black-50' : 'primary' }}" style="grid-row: 1 / 3; grid-column: 2;">
                            <span class="d-block position-abolsute" style="border-left: 2px solid; height: 55%; transform: translate(1.8rem, 3.5rem)">
                                <span class="position-absolute rounded-circle text-center bg-white" style="top: 50%; border: 2px solid; width: 1.75rem; z-index: 1; transform: translate(-50%, -50%); {{ $this->routeEdited ? 'cursor: pointer;' : '' }}" wire:click="resetRoute">
                                    <i class="las la-lock{{ $this->routeEdited ? '-open' : '' }}"></i>
                                </span>
                            </span>
                        </div>
                    </div>
                ...
                <div class="preview">
                    <div class="jumbotron mx-3 px-4 py-3 shadow">
                        ...
                        <p class="font-sm text-monospace text-primary" style="overflow-wrap: break-word;">{{ $this->preview[$value] ?? '' }}</p>
                        @endforeach
                    </div>
                </div>
                @endif
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="d-flex align-items-center btn btn-primary" @disabled(!$this->preview || count($this->errors))><i class="la la-circle-o-notch la-spin d-none"></i>Create</button>
                </div>
            </form>
        </div>
    </div>
